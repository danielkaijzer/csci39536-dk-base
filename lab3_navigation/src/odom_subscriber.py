...
class OdomSubscriber():
    ...
    def callback(self, topic_data: Odometry):
        ...
        # Add your code here!
        (roll,pitch,theta_z) = euler_from_quaternion([orientation.x, orientation.y, orientation.z, orientation.w], 'sxyz')

        # Here we print out the values that we're interested in:
        if self.counter > 10:
            self.counter = 0
            print(f"x = {pos_x:.3f} (m), y = {pos_y:.3f} (m), theta_z = {theta_z:.3f} (radians)")
        else:
            self.counter += 1
